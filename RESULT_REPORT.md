...
- 仅修复 PR #2 合入后的玩家 AABB 碰撞后遗症，不新增合成、生存等玩法功能。
- 重点维护 `src/v3.js` 中的玩家碰撞与树叶碰撞属性。

### 修复内容

1. 修复纵向移动落地判定：
   - 下落时只把当前脚底到下一帧脚底之间实际跨过的方块顶面视为落地面。
   - 如果 AABB 命中了侧面高墙/树干但没有跨过可落地顶面，不再使用命中方块里的最高 `y + 1` 传送玩家。
2. 修复上升碰撞判定：
   - 跳跃上升时只把当前头顶到下一帧头顶之间实际跨过的方块底面视为天花板。
   - 侧面擦碰方块时不再把玩家高度强行改到异常位置。
3. 修复树下靠不到树干：
   - 为树叶和松叶保留 `solid: true` 渲染属性，确保视觉和区块网格生成不变。
   - 增加 `collisionSolid: false`，让玩家碰撞查询忽略 `leaves` / `pineLeaves`，避免低树叶阻挡玩家靠近树干。
